    ...
    public class Autocomplete : Helpers
    {
        ...
        public async Task<AutocompleteResponse> GetAutocompleteResults(AutocompleteRequest autocompleteRequest, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var result = await MakeHttpRequest<AutocompleteResponse>(Options, HttpMethod.Get, url, requestHeaders, cancellationToken: cancellationToken).ConfigureAwait(false);
